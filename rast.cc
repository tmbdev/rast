...
"   maxresults verbose mindx maxdx mindy maxdy amin amax minscale maxscale\n"
...
"   usegrad maxresults verbose error angle_error tolerance angle_tolerance\n"
"   minweight lsq\n"
...
int main_lines(int argc,char **argv) {
	...
	if(fields<2 && angle_error<2*M_PI) throw "no angles available; set angle_error to >=2pi";
...
int main_slines(int argc,char **argv) {
	...
	if(usegrad) a = normangleOf(a-M_PI/2.0);
	if(fields<4 && angle_error<2*M_PI) throw "no angles available; set angle_error to >=2pi";
