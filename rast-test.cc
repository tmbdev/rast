...
for(int TRIAL=0;TRIAL<(N);TRIAL++) { try { \
if(FREQ>0&&TRIAL>0&&TRIAL%FREQ==0) fprintf(stderr," %d",TRIAL); \

#define end_trials \
} \
catch(const char *s) {fprintf(stderr,"\n%s: trial %d failed: %s\n",NAME__,TRIAL,s);} \
...
bool within(float x,float y,float eps) {
    return fabs(x-y)<=eps;
}
...
int iuniform(int low,int high) {
    return lrand48()%(high-low) + low;
}
...
void test_linesp2d_1() {
	...
	vec2 u,v;
	do {u = vuniform(0,0,512,512); v = vuniform(0,0,512,512);} while(distance(u,v)<1.0);
	Segment seg(u,v);
...
void test_rastp2d_1() {
	...
	// these assertions are pretty heuristic given that it's a bounded error match
...
void test_rastp2d_2() {
	...
	rast->match();
	assert(rast->nresults()>0);
	if(0) printf("*** %d: q=%g %g:%g %g %g %g   ---   %g %g %g %g\n",TRIAL,
	       rast->ubound(0),rast->lbound(0),
...
struct seg2 {
    vec2 u,v;
    seg2() {}
    seg2(vec2 u,vec2 v) { this->u = u; this->v = v; }
