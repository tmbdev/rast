...
struct Trie1 {
	...
	for(int i=clamp(x0/eps,buckets.dim(0)),n=clamp(x1/eps,buckets.dim(0));i<=n;i++) {
...
struct Trie2 {
	...
	x1 -= xoffset;
	y1 -= yoffset;
	for(int i=clamp(int(x0/eps),buckets.dim(0)),n=clamp(int(x1/eps),buckets.dim(0));i<=n;i++) {
	    for(int j=clamp(int(y0/eps),buckets.dim(1)),m=clamp(int(y1/eps),buckets.dim(1));j<=m;j++) {
