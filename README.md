...
- Python bindings generated by SWIG (`_rast.so` + `rast.py`).
...
toolchain (C++ compiler, SWIG, Python, NumPy, doctest, clang-format) from
...
`tests`, `_rast.so`, `rast.py`) land under `_build/`. The project root stays
clean.
...
After `pixi run build`, the SWIG-generated module is at `_build/rast.py` /
`_build/_rast.so`. The pixi environment sets `PYTHONPATH=$PIXI_PROJECT_ROOT/_build`
so:
