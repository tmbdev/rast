...
std::unique_ptr<RastSS2D> makeMatcher() {
  std::unique_ptr<RastSS2D> r(makeRastSS2D());
  r->set_maxresults(1);
  r->set_verbose(false);
...
TEST_CASE("RastSS2D: factory yields a usable instance") {
  std::unique_ptr<RastSS2D> r(makeRastSS2D());
  REQUIRE(r != nullptr);
}

TEST_CASE("RastSS2D: identity match of a square outline") {
  auto r = makeMatcher();
  // Constrain the search window so the matcher can't drift to an alternate
  // (rotation- or sliding-) consistent translation around the closed outline.
  r->set_xrange(-5, 5);
  r->set_yrange(-5, 5);
...
TEST_CASE("RastSS2D: pure translation recovered") {
  auto r = makeMatcher();
  const float dx = 25.0f, dy = 40.0f;
...
TEST_CASE("RastSS2D: empty input emits at most a degenerate (ubound==0) match") {
  auto r = makeMatcher();
  r->match();
  if (r->nresults() >= 1) {
...
TEST_CASE("RastSS2D: lbound <= ubound") {
  auto r = makeMatcher();
...
TEST_CASE("RastSS2D: maxresults limits results") {
  auto r = makeMatcher();
  r->set_maxresults(2);
  const std::array<Seg, 4> segs = {
      {{0, 0, 30, 0}, {0, 0, 0, 30}, {30, 0, 30, 30}, {0, 30, 30, 30}}};
  for (const auto& s : segs) {
    r->add_mseg(s.x0, s.y0, s.x1, s.y1);
    r->add_iseg(s.x0, s.y0, s.x1, s.y1);
  }
