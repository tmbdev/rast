...
#if 0
#define ASSERT(X) do{if(!(X)) throw "ASSERTION: " #X;}while(0)
...
namespace lumo_crastrs2d {
	...
	float lsq(float eps,float delta,vec2 a) {
	    float d = p.distance(a);
	    return max(0.0,1.0 - sqr(max(0.0,(d-delta)/(eps*eps))));
	}
    };
	...
	vec2 p,q;
	float a;
	vec2 dir; float l0,l1;
	vec2 normal; float d;
	Segment() {
	}
	Segment(vec2 p,vec2 q) {
	    set(p,q);
	}
	...
	void print(FILE *stream=stdout) {
	    fprintf(stream,
		    ...
		    depth,
		    lbound,ubound,
		    region.low(0),region.high(0),
		    region.low(1),region.high(1),
		    region.low(2),region.high(2),
	...
	void set_qtolerance(float value) {
	    qtolerance = value;
	}
...
RastRS2D *makeRastRS2D() {
    return new lumo_crastrs2d::CRastRS2D();
}
...
struct seg2 {
    ...
    seg2 operator+(vec2 tr) {
	return seg2(u+tr,v+tr);
    }
    float angle() {
	return angleOf(v-u);
    }
...
int main(int argc,char **argv) {
	...
	autodel<crastrs2d> rast = new crastrs2d();
	if(trivial) {
