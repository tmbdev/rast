...
namespace lumo_calignmentp2d {
	...
	void add_ipoint(float x,float y) {
	    image.push(vec2(x,y));
	}
	void clear_mpoints() {
	    model.clear();
	}
	void add_mpoint(float x,float y) {
	    model.push(vec2(x,y));
	}
	...
	float translation(int dim) {
	    return solution.translation[dim];
	}
	float angle() {
	    return normangleOf(angleOf(solution.rotation));
	}
	float scale() {
	    return norm(solution.rotation);
	}
    };
